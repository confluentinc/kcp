...
build:
	...
	go build -ldflags "-X main.commit=$$COMMIT -X main.branch=$$BRANCH -X main.date=$$DATE" -o $(BINARY_NAME) $(MAIN_PATH)
...
build-linux:
	...
	BRANCH=$$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "unknown"); \
	DATE=$$(date -u '+%Y-%m-%dT%H:%M:%SZ'); \
	GOOS=linux GOARCH=amd64 go build -ldflags "-X main.commit=$$COMMIT -X main.branch=$$BRANCH -X main.date=$$DATE" -o $(BINARY_NAME)-linux-amd64 $(MAIN_PATH)

build-linux-arm64:
	@COMMIT=$$(git rev-parse --short HEAD 2>/dev/null || echo "unknown"); \
	BRANCH=$$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "unknown"); \
	DATE=$$(date -u '+%Y-%m-%dT%H:%M:%SZ'); \
	GOOS=linux GOARCH=arm64 go build -ldflags "-X main.commit=$$COMMIT -X main.branch=$$BRANCH -X main.date=$$DATE" -o $(BINARY_NAME)-linux-arm64 $(MAIN_PATH)

build-darwin:
	@COMMIT=$$(git rev-parse --short HEAD 2>/dev/null || echo "unknown"); \
	BRANCH=$$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "unknown"); \
	DATE=$$(date -u '+%Y-%m-%dT%H:%M:%SZ'); \
	GOOS=darwin GOARCH=amd64 go build -ldflags "-X main.commit=$$COMMIT -X main.branch=$$BRANCH -X main.date=$$DATE" -o $(BINARY_NAME)-darwin-amd64 $(MAIN_PATH)

build-darwin-arm64:
	@COMMIT=$$(git rev-parse --short HEAD 2>/dev/null || echo "unknown"); \
	BRANCH=$$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "unknown"); \
	DATE=$$(date -u '+%Y-%m-%dT%H:%M:%SZ'); \
	GOOS=darwin GOARCH=arm64 go build -ldflags "-X main.commit=$$COMMIT -X main.branch=$$BRANCH -X main.date=$$DATE" -o $(BINARY_NAME)-darwin-arm64 $(MAIN_PATH)
