...
func TestTargetInfra_EnvironmentOnly(t *testing.T) {
	service := &TargetInfraHCLService{ResourceNames: NewTerraformResourceNames(), DeploymentID: "testdeploy"}
	request := types.TargetClusterWizardRequest{
		AwsRegion:        "us-east-1",
		NeedsEnvironment: true,
		EnvironmentName:  "standalone-env",
		NeedsCluster:     false,
		PreventDestroy:   true,
	}

	// This may fail validation in API handler, but test generation handles it gracefully
	project := service.GenerateTerraformFiles(request)
	files := projectToFiles(project)
