...
func MigrationInfraDecision(p Profile, tier Tier) MigrationInfraChoice {
	...
	// An Apache Kafka / Confluent Platform source known to run outside AWS (Azure, GCP,
	// or on-premises) can't use kcp's migration-infra, which provisions the outbound
	// cluster link from an AWS VPC. Cluster Linking still applies — only the link's
