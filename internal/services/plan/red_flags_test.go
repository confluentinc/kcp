...
func TestDetectRedFlags_EmptyFleetReturnsNil(t *testing.T) {
	assert.Nil(t, detectRedFlags(report.ProcessedState{}, &types.Plan{}, defaultCfg(t), defaultInputs()))
}
...
func buildPlanForRedFlags(t *testing.T, state report.ProcessedState, cfg *PlanConfig, inputs types.PlanInputsResolved) *types.Plan {
	t.Helper()
