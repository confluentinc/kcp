...
func (s *PlanService) Build(state types.ProcessedState, inputs types.PlanInputsResolved, stateFilePath string) (*types.Plan, error) {
	...
	for _, c := range clusters {
		...
		plan.OpenQuestions = append(plan.OpenQuestions, detectOpenQuestions(c, sizing, ct, net, s.cfg)...)
	}
...
func detectOpenQuestions(c types.ProcessedCluster, sizing types.ClusterSizing, ct types.ClusterTypeDecision, net types.NetworkingDecision, cfg *PlanConfig) []types.OpenQuestion {
	...
	if sizing.Degraded {
		oqs = append(oqs, types.OpenQuestion{
			...
			HowToClose: fmt.Sprintf("Re-run `kcp scan metrics --region %s` to backfill CloudWatch metrics for the affected cluster(s).", c.Region),
		})
	}
	if !aclScanRan(c) && !isServerless(c) {
		oqs = append(oqs, types.OpenQuestion{
			...
			HowToClose: fmt.Sprintf("Re-run `kcp scan clusters --region %s` with admin Kafka credentials (SASL/IAM or SCRAM) so the ACL list populates.", c.Region),
		})
	}
	if brokerInventoryGap(c) {
		oqs = append(oqs, types.OpenQuestion{
			...
			HowToClose: fmt.Sprintf("Re-run `kcp discover --region %s` against the source AWS account to populate the broker inventory.", c.Region),
		})
	}
	if topicCount(c) == 0 {
		oqs = append(oqs, types.OpenQuestion{
			...
			HowToClose: fmt.Sprintf("Re-run `kcp scan clusters --region %s` with admin Kafka credentials to populate the topic list.", c.Region),
		})
	}
