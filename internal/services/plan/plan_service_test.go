...
func TestDetectOpenQuestions(t *testing.T) {
	...
	t.Run("nil ACLs on PROVISIONED (scan didn't run OR --skip-acls) → acls_not_scanned OQ", func(t *testing.T) {
		// Both "admin scan didn't run" and "scan ran with --skip-acls"
		// produce Acls == nil. The OQ fires for both — the customer
		// can't trust the ACL-cap rule against an unknown ACL count.
