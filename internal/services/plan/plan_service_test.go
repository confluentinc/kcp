...
func TestDetectOpenQuestions(t *testing.T) {
	...
	t.Run("missing P95 metrics → missing_p95_metrics OQ", func(t *testing.T) {
		...
		oqs := detectOpenQuestions(c, sizing, ent, plNet("noMetrics"), cfg, types.PlanInputsResolved{SizingPercentile: "p95"})
	...
	t.Run("nil ACLs on PROVISIONED with topics scan → acls_not_scanned suppressed (scan ran with 0 ACLs)", func(t *testing.T) {
		...
		oqs := detectOpenQuestions(c, types.ClusterSizing{ClusterID: "provisioned-zero-acls", FinalECKU: 1}, ent, plNet("provisioned-zero-acls"), cfg, types.PlanInputsResolved{SizingPercentile: "p95"})
		for _, oq := range oqs {
	...
	t.Run("no topics scan + nil ACLs → acls_not_scanned OQ", func(t *testing.T) {
		...
		oqs := detectOpenQuestions(c, types.ClusterSizing{ClusterID: "noAcls", FinalECKU: 1}, ent, plNet("noAcls"), cfg, types.PlanInputsResolved{SizingPercentile: "p95"})
	...
	t.Run("SERVERLESS cluster suppresses acls_not_scanned and broker_inventory_empty", func(t *testing.T) {
		...
		c.KafkaAdminClientInformation.Topics = &types.Topics{Summary: types.TopicSummary{Topics: 5}}
		c.KafkaAdminClientInformation.Acls = nil
		oqs := detectOpenQuestions(c, types.ClusterSizing{ClusterID: "serverless", FinalECKU: 1}, ent, plNet("serverless"), cfg, types.PlanInputsResolved{SizingPercentile: "p95"})
		for _, oq := range oqs {
	...
	t.Run("zero brokers on PROVISIONED → broker_inventory_empty OQ", func(t *testing.T) {
		...
		c.AWSClientInformation.Nodes = nil
		oqs := detectOpenQuestions(c, types.ClusterSizing{ClusterID: "noBrokers", FinalECKU: 1}, ent, plNet("noBrokers"), cfg, types.PlanInputsResolved{SizingPercentile: "p95"})
	...
	t.Run("zero topics → topic_inventory_empty OQ", func(t *testing.T) {
		...
		oqs := detectOpenQuestions(c, types.ClusterSizing{ClusterID: "noTopics", FinalECKU: 1}, ent, plNet("noTopics"), cfg, types.PlanInputsResolved{SizingPercentile: "p95"})
	...
	t.Run("spiky workload does NOT generate an OQ (FYI only)", func(t *testing.T) {
		...
		oqs := detectOpenQuestions(c, sizing, ent, plNet("spiky"), cfg, types.PlanInputsResolved{SizingPercentile: "p95"})
	...
	t.Run("fully populated cluster emits no OQs", func(t *testing.T) {
		...
		oqs := detectOpenQuestions(c, types.ClusterSizing{ClusterID: "complete", FinalECKU: 2}, ent, plNet("complete"), cfg, types.PlanInputsResolved{SizingPercentile: "p95"})
