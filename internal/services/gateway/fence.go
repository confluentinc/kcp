...
// FencedRouteNames returns the spec.routes[].name values that carry a
// non-nil fence block — the inverse of FenceRoutes. It exists to recover
// route names from a pre-38f5c974 fenced-CR snapshot (the file FenceRoutes
// replaced): a migration-state.json written by that older kcp has no
// fence_routes to resume from, and this reconstructs it from the fenced CR it
// did snapshot. See MigrationConfig.UnmarshalJSON in the migration package.
func FencedRouteNames(fencedCRBytes []byte) ([]string, error) {
	var obj map[string]any
	if err := yaml.Unmarshal(fencedCRBytes, &obj); err != nil {
		return nil, fmt.Errorf("parsing legacy fenced gateway CR: %w", err)
	}

	spec, ok := mapField(obj, "spec")
	if !ok {
		return nil, fmt.Errorf("legacy fenced gateway CR has no spec")
	}
	routes, ok := sliceField(spec, "routes")
	if !ok {
		return nil, fmt.Errorf("legacy fenced gateway CR has no spec.routes")
	}
	...
	for _, raw := range routes {
		...
		// A nulled `fence` (fence: null) counts as unfenced, matching
		// FenceRoutesObj's own semantics above.
		if fence, has := route["fence"]; !has || fence == nil {
			continue
		}
		name, ok := stringField(route, "name")
		if !ok || name == "" {
			return nil, fmt.Errorf("legacy fenced gateway CR has a fenced route with no name; cannot recover its identity")
		}
		names = append(names, name)
	}
	if len(names) == 0 {
		return nil, fmt.Errorf("legacy fenced gateway CR has no routes carrying a fence block")
	}
	return names, nil
}
