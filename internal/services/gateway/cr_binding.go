...
// CR's route-scoped binding shape — there is no manifest mode field.
type RouteMode int
...
// route-scoped binding: a singular streamingDomain object ⇒ static; a plural
// streamingDomains array ⇒ dynamic. The two are mutually exclusive per route
// (the CRD enforces this with a CEL XOR). A route absent from the CR, or a
// hand-edited route carrying neither or both bindings, is an error — mode is
...
// The resolution is three-way: zero ids (the domain is absent from
...
// more than one (a multi-homed domain) is a hard error naming the domain and its
// ids, since this manifest shape carries no disambiguator.
