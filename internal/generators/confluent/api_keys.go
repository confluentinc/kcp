...
func GenerateSchemaRegistryAPIKey(envName string, isNewEnv bool) *hclwrite.Block {
	...
	ownerBlock.Body().SetAttributeRaw("id", hcl.TokensForResourceReference("confluent_service_account.app-manager.id"))
	ownerBlock.Body().SetAttributeRaw("api_version", hcl.TokensForResourceReference("confluent_service_account.app-manager.api_version"))
	ownerBlock.Body().SetAttributeRaw("kind", hcl.TokensForResourceReference("confluent_service_account.app-manager.kind"))
	apiKeyBlock.Body().AppendBlock(ownerBlock)
	apiKeyBlock.Body().AppendNewline()

	managedResourceBlock := hclwrite.NewBlock("managed_resource", nil)
	managedResourceBlock.Body().SetAttributeRaw("id", hcl.TokensForResourceReference("data.confluent_schema_registry_cluster.schema_registry.id"))
	managedResourceBlock.Body().SetAttributeRaw("api_version", hcl.TokensForResourceReference("data.confluent_schema_registry_cluster.schema_registry.api_version"))
	managedResourceBlock.Body().SetAttributeRaw("kind", hcl.TokensForResourceReference("data.confluent_schema_registry_cluster.schema_registry.kind"))

	environmentApiKeyBlock := hclwrite.NewBlock("environment", nil)
	envRef := GetEnvironmentReference(isNewEnv)
	environmentApiKeyBlock.Body().SetAttributeRaw("id", hcl.TokensForResourceReference(envRef))
...
func GenerateKafkaAPIKey(envName string, isNewEnv bool) *hclwrite.Block {
	...
	managedResourceBlock := hclwrite.NewBlock("managed_resource", nil)
	managedResourceBlock.Body().SetAttributeRaw("id", hcl.TokensForResourceReference("confluent_kafka_cluster.cluster.id"))
	managedResourceBlock.Body().SetAttributeRaw("api_version", hcl.TokensForResourceReference("confluent_kafka_cluster.cluster.api_version"))
	managedResourceBlock.Body().SetAttributeRaw("kind", hcl.TokensForResourceReference("confluent_kafka_cluster.cluster.kind"))
	...
	apiKeyBlock.Body().SetAttributeRaw("depends_on", hcl.TokensForList([]string{
