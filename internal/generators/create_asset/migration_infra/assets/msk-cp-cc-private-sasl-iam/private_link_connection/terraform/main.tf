...
resource "confluent_private_link_attachment" "aws" {
  display_name = "msk-mig-privatelink-attachment-${random_string.suffix.result}"
  cloud        = "AWS"
...
resource "confluent_private_link_attachment_connection" "aws" {
  display_name = "msk-mig-privatelink-connection-${random_string.suffix.result}"
  environment {
    id = var.confluent_cloud_environment_id
  }
...
resource "aws_route53_zone" "private" {
  ...
  tags = {
    Name = "msk-mig-private-dns-zone-${random_string.suffix.result}"
  }
}
