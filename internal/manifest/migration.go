...
type TargetKafka struct {
	...
	// ClusterCredentials is the destination KAFKA leg: sasl_plain, sasl_scram,
	// mtls, unauthenticated_tls, or unauthenticated_plaintext (iam is rejected —
	// the destination is Confluent Cloud/Platform, never MSK). Used by kind:
	// GatewayMigration only; kind: Migration rejects it.
	ClusterCredentials CredentialsRef `yaml:"clusterCredentials,omitempty" json:"clusterCredentials,omitempty"`
}
