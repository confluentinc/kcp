#!/bin/bash
# Generate TLS certificates for the Apache Kafka scan test.
...
keytool -genkey -noprompt \
    -alias kafka-server \
    -dname "CN=apache-kafka,OU=Test,O=Test,L=San Francisco,S=CA,C=US" \
...
subjectAltName = DNS:apache-kafka,DNS:localhost,IP:127.0.0.1
EOF
...
openssl req -new -x509 -nodes -keyout prometheus-server.key -out prometheus-server.crt \
    ...
    -addext "subjectAltName=DNS:localhost,DNS:apache-kafka-prometheus-tls,IP:127.0.0.1" 2>/dev/null
