#!/bin/bash
# Start the Apache Kafka scan broker and create test data.
...
# Start the broker
echo "Starting Apache Kafka scan broker..."
docker compose -f "$SCRIPT_DIR/docker-compose.yml" up -d

# Wait for Kafka to be ready on the internal plaintext listener
echo "Waiting for Kafka to be ready..."
CONTAINER_NAME="kcp-test-apache-kafka"
MAX_WAIT=60
WAIT_TIME=0

while [ $WAIT_TIME -lt $MAX_WAIT ]; do
    if docker exec $CONTAINER_NAME kafka-broker-api-versions --bootstrap-server apache-kafka:29092 > /dev/null 2>&1; then
        echo "Kafka is ready!"
        break
    fi
...
docker exec -e KAFKA_OPTS= $CONTAINER_NAME kafka-topics --bootstrap-server apache-kafka:29092 \
...
for BROKER_CONTAINER in kcp-test-apache-kafka-sasl-sha512-only kcp-test-apache-kafka-jmx-auth kcp-test-apache-kafka-jmx-tls; do
...
docker exec -e KAFKA_OPTS= $CONTAINER_NAME kafka-acls --bootstrap-server apache-kafka:29092 \
...
docker wait kcp-test-apache-kafka-prometheus-seeder kcp-test-apache-kafka-prometheus-auth-seeder kcp-test-apache-kafka-prometheus-tls-seeder > /dev/null
echo "Restarting Prometheus instances to load seeded TSDB blocks..."
docker restart kcp-test-apache-kafka-prometheus kcp-test-apache-kafka-prometheus-auth kcp-test-apache-kafka-prometheus-tls > /dev/null
sleep 3
