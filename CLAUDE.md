...
│   ├── migrate-connectors
│   └── reverse-proxy
├── cutover          # Cutover execution workflow
│   ├── init         # Initialize cutover state
│   ├── list         # List cutover sessions
...
4. **`cutover-state.json`**: Tracks cutover execution state (created by `kcp cutover init`).
...
# 8. Initialize cutover
kcp cutover init --state-file kcp-state.json --cluster-arn <arn>

# 9. Execute cutover
kcp cutover execute --state-file kcp-state.json
```
...
# 3. Continue with standard workflow (same as MSK steps 4-9)
kcp ui
# ... etc
```
