...
KCP is a CLI tool for planning and executing Kafka migrations from **AWS MSK and Apache Kafka** to Confluent Cloud. It provides commands for discovering resources, generating reports, creating migration assets (Terraform), and managing the migration workflow.
...
- **Code location**: `internal/sources/apachekafka/`
- **Workflow**: Direct Kafka Admin API scanning
- **Test environments**: Docker Compose in `integration-tests/apache-kafka-scan/`
...
go test ./internal/sources/apachekafka -v
...
A single Docker Compose environment (`integration-tests/apache-kafka-scan/`) runs all Apache Kafka test variants via `make test-apache-kafka-scan`. It starts a multi-listener KRaft Kafka broker, two JMX-specific brokers, three Prometheus instances, and producer/consumer containers for traffic generation.
...
- TLS: Certificates generated by `integration-tests/apache-kafka-scan/generate-certs.sh`
- Credential files: `integration-tests/apache-kafka-scan/credentials/*.yaml`
...
- **ApacheKafkaSource** (`internal/sources/apachekafka/`): Implements Kafka Admin API scanning for Apache Kafka
...
- Results stored as `ClusterMetrics` on `ApacheKafkaDiscoveredCluster` in state file.
