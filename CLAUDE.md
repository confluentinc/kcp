...
The CLI follows a hierarchical Cobra command structure: `discover`, `scan`, `report`, `create-asset`, `migrate`, `cutover`, `ui`, `update`, `version`. Run `kcp --help` for the full tree. (`migration` remains as a hidden deprecation stub pointing to `cutover`.)
...
- `cutover-state.json` — created by `kcp cutover init`.
