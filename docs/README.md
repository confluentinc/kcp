...
> [!NOTE]
> **For OSK (Open Source Kafka) migrations:** AWS authentication is not required. OSK clusters are accessed directly via Kafka Admin API using credentials you provide in the `osk-credentials.yaml` file. See the [OSK Usage](#kcp-scan-clusters) section for authentication configuration details.
...
- `--skip-managed-connectors`: Skips the MSK Connect connector discovery. Useful when you don't have `kafkaconnect:ListConnectors` and `kafkaconnect:DescribeConnector` permissions or are using self-managed Kafka Connect instead of MSK Connect.
...
`kcp discover --region us-east-1 --skip-topics --skip-costs --skip-metrics --skip-managed-connectors`

The command will produce an `msk-credentials.yaml` and a `kcp-state.json` file. An overview will be output to the terminal and a more in-depth breakdown can be seen in the [UI](#kcp-ui).
...
> [!NOTE]
> **Graceful degradation:** If the `MSKNonCriticalPermissions` or `MSKConnectPermissions` are missing, discovery will log a warning and continue without that data rather than failing the entire cluster. This means you can run `kcp discover` with a minimal permission set and still get core cluster information.
>
> **Skip flags and permissions:**
>
...
> - If using `--skip-managed-connectors`, the `MSKConnectPermissions` are not required
...
Scan Kafka clusters at the Kafka level to discover topics, ACLs, and configurations. This command works with both AWS MSK and Open Source Kafka (OSK) clusters.

> [!NOTE]
> Optionally, you can provide the kcp user/principal connecting to the cluster with permissions to read the `connect-configs` and `connect-status` topics. This allows kcp to aggregate discovered self-managed connectors, their connect URL, state and config.

**Required Flags**

- `--source-type`: The source cluster type (`msk` or `osk`)
- `--credentials-file`: Path to the credentials file (`msk-credentials.yaml` for MSK or `osk-credentials.yaml` for OSK)
...
For MSK clusters, use the `msk-credentials.yaml` file generated by `kcp discover`. The discover command automatically sets the SCRAM mechanism to `SHA512` for MSK clusters, as AWS MSK only supports SCRAM-SHA-512 (not SHA-256).
...
Kafka exposes operational metrics via JMX (Java Management Extensions), a Java-native monitoring protocol. Since kcp is written in Go, it cannot connect to JMX directly. Instead, it uses [Jolokia](https://jolokia.org/) — a lightweight JVM agent that exposes JMX MBeans as a JSON REST API over HTTP. Jolokia must be installed on each Kafka broker as a JVM agent:

```
KAFKA_OPTS="-javaagent:/path/to/jolokia-agent.jar=port=8778,host=0.0.0.0"
```

Each broker runs its own Jolokia endpoint. To collect metrics from a multi-broker cluster, list all broker endpoints in the credentials file.

**Counter-Based Rate Calculation**

Kafka's JMX rate metrics (such as `BytesInPerSec`) expose several pre-computed fields including `OneMinuteRate`, `FiveMinuteRate`, and `FifteenMinuteRate`. These are **exponentially weighted moving averages (EWMA)** — mathematical constructs that continuously smooth incoming data with a decay function. While useful for dashboards, EWMA values are not suitable for aggregation: consecutive readings are highly correlated, and computing min/max/average over a series of EWMA values would understate the true variance in traffic.

Instead, kcp reads the `Count` field — a monotonic counter that tracks the total number of bytes (or messages) since the broker started. By taking two consecutive `Count` readings and dividing the difference by the elapsed time, kcp computes the **actual rate** for that interval:

```
rate = (Count_current - Count_previous) / elapsed_seconds
```

This approach produces independent data points with real variance that accurately reflect traffic patterns over each polling interval.
...
- **TLS**: Set `tls.ca_cert` for certificate verification, or `tls.insecure_skip_verify: true` for self-signed certificates. Can be combined with password authentication.
...
- `id`: Unique identifier for the cluster (used in state file)
- `bootstrap_servers`: List of broker addresses (host:port)
- `auth_method`: Authentication configuration (choose one):
  - `sasl_scram`: SASL/SCRAM-SHA-256 or SASL/SCRAM-SHA-512 authentication
    - `username`: SASL username
    - `password`: SASL password
    - `mechanism`: (Optional) SCRAM mechanism - `"SHA256"` or `"SHA512"`. Defaults to `"SHA256"` for OSK. **Note:** AWS MSK only supports `"SHA512"` (automatically set by `kcp discover`)
  - `tls`: TLS/mTLS authentication
