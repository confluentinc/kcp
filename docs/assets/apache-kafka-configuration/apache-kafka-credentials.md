---
title: Apache Kafka credentials file
---
...
!!! note "SCRAM mechanism for Apache Kafka vs MSK"
    ...
    Apache Kafka supports both `SHA256` and `SHA512`. `SHA256` is the more common open-source default, so `kcp` does not infer one for you — set `mechanism` explicitly.
