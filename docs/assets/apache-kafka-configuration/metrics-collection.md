---
title: Metrics collection (Apache Kafka)
---
...
See the [`apache-kafka-credentials.yaml` reference](apache-kafka-credentials.md) for the full schema.
...
pods (e.g. `apache-kafka-kraftcontroller-*`) that require their own `PodMonitor` or
`ServiceMonitor`.
...
cluster. See the [`prometheus` field reference](apache-kafka-credentials.md#prometheus--optional-for-historical-metrics)
for details.
