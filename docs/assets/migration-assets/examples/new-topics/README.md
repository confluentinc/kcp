...
count and explicitly-set (non-default) configs. Topics are created at the
target's default replication factor (Confluent Cloud uses RF=3). It needs **no
cluster link**, so there is no `spec.clusterLink` block.
