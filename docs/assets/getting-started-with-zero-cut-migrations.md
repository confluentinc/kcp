...
| **Verify fence**      | _(Experimental, opt-in)_ With `--detect-unrouted-producers-duration` set, monitors source offsets over that window to catch producers bypassing the gateway; on detection, unblocks and restores offset sync automatically. Defaults to `0` (skipped) — traffic that is slow but legitimate can otherwise trip a false positive | Still retrying                                                          |
