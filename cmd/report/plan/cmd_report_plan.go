...
func NewReportPlanCmd() *cobra.Command {
  ...
  kcp report plan --state-file kcp-state.json --output json`,
	...
	reportPlanCmd.SetUsageFunc(func(c *cobra.Command) error {
		// Skip re-printing c.Short — the Long string above already opens
		// with the same sentence (Cobra renders Long first), so dropping
		// the duplicate puts the description at the top where readers
		// expect it and avoids the mid-help repeat.
