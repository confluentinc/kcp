...
group- or world-readable.`,
  ...
  # Kubernetes validation entirely (no live calls of any kind)
  kcp migration init --migration-yaml gateway-migration.yaml --skip-validate`,
	...
	migrationInitCmd.Flags().BoolVar(&skipValidate, "skip-validate", false, "Skip credential resolution and destination/Kubernetes resource validation entirely — no live calls of any kind are made. Useful for testing.")
...
func runMigrationInit(cmd *cobra.Command, args []string) error {
	...
	// Route and TargetDomain are pure manifest data — no live call needed for
	// either. Topics/FenceYAML/SwitchoverYAML/GatewayYAML/Mode are NOT set
	// here: they require migplan.Reconcile (a live call), which only runs on
	// the non-skip-validate path (Phase 5 below) — MigrationActions.Initialize
	// is what sets them, mirroring TBMActions.Initialize's own shape, not this
	// command directly.
	...
	// migplan.Reconcile resolves route mode, runs preconditions (staged-auth,
	// secret existence, cluster-identity checks), and derives the fence/
	// switchover fragments — replacing this command's own former
	// ResolveRouteMode/DeriveBootstrapServerID/staticTopicsOf derivation and
	// Initialize's own former CheckRedundantAuthStaged/cluster-link topic
	// classification. Only reached on the non-skip-validate path — this is
	// the one live call --skip-validate exists specifically to avoid.
	res, err := migplan.Reconcile(cmd.Context(), g)
	if err != nil {
		return fmt.Errorf("failed to produce the reconcile plan: %w", err)
	}
	// Dynamic-route migrations are refused here, not by migplan itself:
	// Reconcile happily reconciles either mode, but AAO's execute path only
	// implements the static (all-at-once) workflow today. Reconcile's
	// dynamic-path preconditions run and are discarded when the route turns
	// out dynamic — an accepted, harmless cost (read-only), not a correctness
	// concern.
	if res.Mode == "dynamic" {
