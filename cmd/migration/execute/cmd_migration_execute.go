...
the migration state file and must be provided each time.`,
  ...
  kcp migration execute \
	...
	optionalFlags.DurationVar(&rolloutTimeout, "rollout-timeout", 0, "Maximum time to wait for the Confluent operator to report the gateway as Ready during fence and switchover. 0 (the default) means no deadline on the readiness waits — they run until the operator converges or the user cancels. The step that waits for the operator to acknowledge the spec is always bounded (5m when this is 0), since a refusal it cannot recognise would otherwise never end.")
