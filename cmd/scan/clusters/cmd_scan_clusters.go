...
The command appends discovered topics, ACLs, the Kafka cluster ID, and (if collected) cluster metrics to each cluster's entry in the state file. With sufficient permissions, kcp also reads the connect-status and connect-configs topics to discover self-managed connectors and their state.`,
