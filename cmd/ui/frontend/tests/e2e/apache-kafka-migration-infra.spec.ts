...
test.describe('Apache Kafka Migration Infrastructure Wizard', () => {
  ...
  test('Public path - generates Terraform for Apache Kafka cluster', async ({ page }) => {
    ...
    await page.fill('#root_cluster_link_name', 'apache-kafka-to-cc-test-link')
  ...
  test('External Outbound path - generates Terraform for Apache Kafka cluster', async ({ page }) => {
    await page.locator('text=production-kafka-us-east').click()
    await page.waitForTimeout(500)
    ...
    await page.fill('#root_target_cluster_id', 'lkc-test123')
    await page.fill('#root_target_rest_endpoint', 'https://test.confluent.cloud:443')
    await page.fill('#root_cluster_link_name', 'apache-kafka-ext-outbound-link')
  ...
  test('Jump Cluster path - no IAM option, goes directly to SASL/SCRAM', async ({ page }) => {
    ...
    // Jump cluster networking - fill ALL required fields (no defaults for Apache Kafka)
    await page.fill('#root_vpc_id', 'vpc-test123')
    ...
    await page.fill('#root_target_cluster_id', 'lkc-test123')
    await page.fill('#root_target_rest_endpoint', 'https://test.confluent.cloud:443')
    await page.fill('#root_cluster_link_name', 'apache-kafka-jump-cluster-link')
    await page.fill('#root_target_environment_id', 'env-test123')
