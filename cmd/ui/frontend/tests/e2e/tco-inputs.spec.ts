...
test.describe('TCO Inputs Page', () => {
  ...
  test('shows MSK and Apache Kafka tabs', async ({ page }) => {
    await expect(page.locator('button:has-text("MSK")')).toBeVisible()
    await expect(page.locator('button:has-text("Apache Kafka")')).toBeVisible()
  })
  ...
  test('Apache Kafka tab shows Apache Kafka cluster with metadata', async ({ page }) => {
  ...
  test('tabs with data do not show empty state message', async ({ page }) => {
    ...
    await page.locator('button:has-text("Apache Kafka")').click()
    await expect(page.locator('text=No Apache Kafka clusters found')).not.toBeVisible()
  })

  test('Apache Kafka label pills are rendered', async ({ page }) => {
    await page.locator('button:has-text("Apache Kafka")').click()
