...
export const createMigrationInfraApacheKafkaWizardConfig = (clusterId: string): WizardConfig => {
  const cluster = getApacheKafkaClusterDataById(clusterId)
  ...
  const sourceKafkaBrokers = (cluster?.bootstrap_servers || []).map((server: string, index: number) => {
    ...
    return {
      broker_id: `apache-kafka-broker-${index}`,
      subnet_id: '',
  ...
  return {
    id: 'migration-infra-apache-kafka-wizard',
    ...
    states: {
      ...
      public_cluster_link_inputs: {
        meta: {
          ...
          uiSchema: {
            ...
            cluster_link_name: {
              'ui:placeholder': 'e.g., apache-kafka-to-cc-migration-link',
            },
            source_cluster_id: {
      ...
      // External outbound path - all fields editable for Apache Kafka (no AWS defaults)
      ...
      // Jump cluster: networking inputs - all editable for Apache Kafka (no AWS defaults)
      jump_cluster_networking_inputs: {
        ...
        on: {
          // Apache Kafka goes directly to SASL/SCRAM auth (no IAM path)
          NEXT: {
      ...
      // Jump cluster: SASL/SCRAM auth (direct - no auth question for Apache Kafka)
