...
func NewBastionHostCmd() *cobra.Command {
	bastionHostCmd := &cobra.Command{
		...
		Long: `Create Terraform assets for deploying a bastion host in AWS within an existing VPC with the latest kcp binary pre-installed. Use this when your source Kafka cluster (MSK or Apache Kafka) is not reachable from the machine running kcp and you do not already have a jump server.
